...
void finalBlock(union BlockBuffer *buffer, size_t bytes, uint64_t totalBits)
{
    ...
    if (byte < 112 && byte > 0)
    {
        ...
        buffer->blocks[block].qwords[15] = chgEndUInt64(totalBits);
    }
    ...
    else if (byte < 128 && byte > 0)
    {
        ...
        if (block < BUFFER_SIZE - 1)
        {
            ...
            buffer->blocks[block + 1].qwords[15] = chgEndUInt64(totalBits);
        }
    }
...
void addPadding(union BlockBuffer *buffer, uint64_t totalBits, size_t addEOS)
{
    for (size_t i = 0; i < 14; i++)
    {
    ...
    buffer->blocks[0].qwords[15] = chgEndUInt64(totalBits);
}
...
void sha512(char *filePath, int iter)
{
    int64_t totalBytes;
    uint64_t totalBits;
    size_t bytes;
...
int main(int argc, char *argv[])
{
    if (argc != 3)
    {
        printf("Usage: ./sha512_c <File Path> <Iterations>\n");
        exit(1);
    }

    sha512(argv[1], atoi(argv[2]));
